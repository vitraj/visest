...
def plot(bare_estimator, ell_mean, var_squared, file_path, img_dpi):
    ...
    # axs[0].plot(ell_mean, bare_estimator, color= '#1f77b4', label= 'Gaussian primary beam')
    axs[0].errorbar(
        ell_mean,
        bare_estimator,
        np.sqrt(var_squared),
    ...
    # axs[0].scatter(ell_mean, 5.13*1e-4 * (1000/ ell_mean)**2.34, s=10, color= '#ff7f0e')
    axs[0].plot(
        ell_mean,
        5.13 * 1e-4 * (1000 / ell_mean) ** 2.34,
        color="k",
        label="Model",
        alpha=0.75,
    )
    ...
    axs[0].set_ylim(
        5.13 * 1e-4 * (1000 / ell_mean[-1]) ** 2.34,
        5.13 * 1e-4 * (1000 / ell_mean[0]) ** 2.34,
    )
    axs[0].grid()
    ...
    axs[1].errorbar(
        ell_mean,
        ell_mean * (ell_mean + 1) * bare_estimator / (2 * np.pi),
        ell_mean * (ell_mean + 1) * np.sqrt(var_squared) / (2 * np.pi),
        color="#1f77b4",
        label="Gaussian primary beam",
        elinewidth=0.75,
        capsize=4,
    ...
    axs[1].set_ylim(
        ...
        * 5.13
        * 1e-4
        * (1000 / ell_mean[-1]) ** 2.34
        / (2 * np.pi),
        ell_mean[0]
        * (ell_mean[0] + 1)
        * 5.13
        * 1e-4
        * (1000 / ell_mean[0]) ** 2.34
        / (2 * np.pi),
    )
...
def setup(config):
    ...
    bare_estimator, ell_mean, var_squared = bare_estimator_func_v4(
        filter_bls,
        filter_vis,
        rshift,
